...
func (p *PolicySet) UnmarshalJSON(b []byte) error {
	...
	for k, v := range jsonPolicySet.StaticPolicies {
		policies[PolicyID(k)] = newPolicy((*ast.Policy)(v))
	}
...
func indexAction(idx *policyIndex, id PolicyID, scope ast.IsActionScopeNode) {
	switch s := scope.(type) {
...
func indexPrincipal(idx *policyIndex, id PolicyID, scope ast.IsPrincipalScopeNode) {
	switch s := scope.(type) {
	case ast.ScopeTypeAll:
...
func indexResource(idx *policyIndex, id PolicyID, scope ast.IsResourceScopeNode) {
	switch s := scope.(type) {
	case ast.ScopeTypeAll:
