			...
			"entityTypes": {
				"User": {
					"shape": {
						...
						"attributes": {
							"name": {"type": "String"},
							"address": {"type": "EntityOrCommon", "name": "Address"}
						}
					}
				}
			},
			...
			"entityTypes": {
				"User": {
					"shape": {
						"type": "Record",
						"attributes": {
							"homeAddress": {"type": "EntityOrCommon", "name": "Address", "required": true}
						}
					}
				}
			},
...
func TestDeduplicatedMemberOfTypes(t *testing.T) {
	...
	// Upstream does not deduplicate memberOfTypes — duplicates are allowed
	// but semantically redundant. Verify at least one entry exists.
	if len(userInfo.MemberOfTypes) == 0 {
		t.Errorf("Expected at least 1 memberOfType, got 0")
	}
}
...
func TestDeduplicatedPrincipalResourceTypes(t *testing.T) {
	...
	// Upstream does not deduplicate principal/resource types — duplicates
	// are allowed but semantically redundant.
	if len(info.PrincipalTypes) == 0 {
		t.Error("Expected at least 1 principal type")
	}
	if len(info.ResourceTypes) == 0 {
		t.Error("Expected at least 1 resource type")
	}
}
...
func TestParseActionMemberOfWithType(t *testing.T) {
	...
	if len(info.MemberOf) != 1 {
		t.Errorf("Expected 1 memberOf entry, got %d", len(info.MemberOf))
	}
		...
		"": {
			"commonTypes": {
				...
				"Person": {
					...
					"attributes": {
						"name": {"type": "String", "required": true},
						"address": {"type": "EntityOrCommon", "name": "Address", "required": true}
					}
				}
			},
			"entityTypes": {
				"User": {
					"shape": {
						"type": "Record",
						"attributes": {
							"profile": {"type": "EntityOrCommon", "name": "Person", "required": true}
						}
					}
				},
				"Resource": {}
...
// TestParseSetTypeNilElement tests that Set type without element is rejected.
func TestParseSetTypeNilElement(t *testing.T) {
	// Upstream rejects Set type without element specification at parse time.
	schemaJSON := `{
		"": {
			"entityTypes": {
				"User": {
	...
	_, err := schema.NewFromJSON([]byte(schemaJSON))
	if err == nil {
		t.Fatal("Expected schema parsing to fail for Set without element")
	}
	if !strings.Contains(err.Error(), "set type missing element") {
		t.Errorf("Expected error about missing element, got: %v", err)
	}
}

// TestParseSetTypeWithElement tests Set type with element parses correctly.
func TestParseSetTypeWithElement(t *testing.T) {
	schemaJSON := `{
		"": {
			"entityTypes": {
				"User": {
	...
	// Element should be StringType
	if _, ok := setType.Element.(schema.StringType); !ok {
		t.Errorf("Expected StringType element, got %T", setType.Element)
	}
}
