...
func TestSchemaValidation_UnknownTypeInPrincipalTypes(t *testing.T) {
	// Unknown types in principalTypes are now rejected at schema parse time
	// by upstream's resolver. This is stricter than the old behavior
	// where parsing succeeded and validation happened at validator creation.
	schemaJSON := `{
		"": {
			"entityTypes": {
				"User": {}
	...
	if err == nil {
		t.Error("Expected schema parsing to fail for unknown principalType")
	}
...
func TestSchemaValidation_UnknownTypeInResourceTypes(t *testing.T) {
	// Unknown types in resourceTypes are now rejected at schema parse time
	// by upstream's resolver.
	schemaJSON := `{
		"": {
			"entityTypes": {
				"User": {}
	...
	_, err := schema.NewFromJSON([]byte(schemaJSON))
	if err == nil {
		t.Error("Expected schema parsing to fail for unknown resourceType")
	}
	if !strings.Contains(err.Error(), "NonExistent") {
		t.Errorf("Expected error mentioning NonExistent, got: %v", err)
	}
...
func TestSchemaValidation_UnknownTypeInMemberOf(t *testing.T) {
	// Unknown types in memberOfTypes are now rejected at schema parse time
	// by upstream's resolver.
	schemaJSON := `{
		"": {
			"entityTypes": {
	...
	_, err := schema.NewFromJSON([]byte(schemaJSON))
	if err == nil {
		t.Error("Expected schema parsing to fail for unknown memberOfType")
	}
	if !strings.Contains(err.Error(), "NonExistent") {
		t.Errorf("Expected error mentioning NonExistent, got: %v", err)
	}
