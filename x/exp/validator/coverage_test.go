				...
				"entityTypes": {
					"Container": {
						"shape": {
							"type": "Record",
							"attributes": {
								"item": {"type": "Entity", "name": "Container"}
							}
						}
					}
				}
				...
				"entityTypes": {
					"User": {
						"shape": {
							"type": "Record",
							"attributes": {
								"data": {"type": "Set", "element": {"type": "String"}}
							}
						}
					}
				}
		...
		"entityTypes": {
			"User": {
				"shape": {
					"type": "Record",
					"attributes": {
						"manager": {"type": "EntityOrCommon", "name": "User"}
					}
				}
			}
		}
...
func TestCommonTypeReference(t *testing.T) {
		...
		"entityTypes": {
			"User": {
				"shape": {
					...
					"attributes": {
						"name": {"type": "EntityOrCommon", "name": "Name"},
						"age": {"type": "EntityOrCommon", "name": "Age"}
					}
				}
			}
		}
	...
	// Verify common types were resolved into entity attributes
	userInfo := v.entityTypes["User"]
	if nameAttr, ok := userInfo.Attributes["name"]; !ok {
		t.Error("Attribute 'name' not found on User")
	} else if _, ok := nameAttr.Type.(schema.StringType); !ok {
		t.Errorf("Expected StringType for 'name' (resolved from Name common type), got %T", nameAttr.Type)
	}
	if ageAttr, ok := userInfo.Attributes["age"]; !ok {
		t.Error("Attribute 'age' not found on User")
	} else if _, ok := ageAttr.Type.(schema.LongType); !ok {
		t.Errorf("Expected LongType for 'age' (resolved from Age common type), got %T", ageAttr.Type)
	}
}
...
func TestDuplicatePrincipalResourceTypes(t *testing.T) {
	...
	// Verify principal and resource types are present (at least 1 each)
	actionInfo := v.actionTypes[types.EntityUID{Type: "Action", ID: "view"}]
	if len(actionInfo.PrincipalTypes) < 1 {
		t.Errorf("Expected at least 1 principal type, got %d", len(actionInfo.PrincipalTypes))
	}
	if len(actionInfo.ResourceTypes) < 1 {
		t.Errorf("Expected at least 1 resource type, got %d", len(actionInfo.ResourceTypes))
	}
}
...
func TestWithAllowUnknownEntityTypes(t *testing.T) {
	...
	// Should succeed - all entity types are defined
	v, err := New(s)
	if err != nil {
	...
	// With the option, should also succeed
	v2, err := New(s, WithAllowUnknownEntityTypes())
	if err != nil {
		...
		"entityTypes": {
			"User": {
				"shape": {
					"type": "Record",
					"attributes": {
						"ref": {"type": "Entity", "name": "User", "required": true}
					}
				}
			}
		},
...
// TestUnspecifiedTypeAttribute tests parsing of attributes with a known type.
func TestUnspecifiedTypeAttribute(t *testing.T) {
	// Schema with an attribute that has a String type
	schemaJSON := `{
		"entityTypes": {
			"User": {
				"shape": {
	...
	// Check that the attribute type is StringType
	userInfo := v.entityTypes["User"]
	dataAttr := userInfo.Attributes["data"]
	if _, ok := dataAttr.Type.(schema.StringType); !ok {
		t.Errorf("Expected StringType, got %T", dataAttr.Type)
	}

	// Test that comparison with StringType works
	policies := cedar.NewPolicySet()
	var policy cedar.Policy
	...
	result := v.ValidatePolicies(policies)
	if !result.Valid {
		t.Errorf("Expected valid policy for StringType comparison, got errors: %v", result.Errors)
	}
}

// TestUnspecifiedTypeInCondition tests that String type in boolean context produces error.
func TestUnspecifiedTypeInCondition(t *testing.T) {
	...
	// Using String type directly as a boolean condition should error
	policies := cedar.NewPolicySet()
	var policy cedar.Policy
	...
	result := ValidatePolicies(s, policies)
	// Using String type as boolean condition should fail
	if result.Valid {
		t.Error("Expected error when using String type as boolean condition")
	}
}
		...
		"entityTypes": {
			"User": {
				"shape": {
					"type": "Record",
					"attributes": {
						"name": {"type": "EntityOrCommon", "name": "MyString", "required": true}
					}
				}
			}
		},
...
func TestParseJSONTypeCommonTypeBool(t *testing.T) {
		...
		"entityTypes": {
			"User": {
				"shape": {
					"type": "Record",
					"attributes": {
						"active": {"type": "EntityOrCommon", "name": "Flag", "required": true}
					}
				}
			}
		},
	...
	// Verify the common type was resolved to BoolType on the attribute
	userInfo := v.entityTypes["User"]
	if activeAttr, ok := userInfo.Attributes["active"]; !ok {
		t.Error("Attribute 'active' not found on User")
	} else if _, ok := activeAttr.Type.(schema.BoolType); !ok {
		t.Errorf("Expected BoolType for 'active' (resolved from Flag common type), got %T", activeAttr.Type)
	}
}
...
func TestParseJSONTypeCommonTypeSet(t *testing.T) {
		...
		"entityTypes": {
			"User": {
				"shape": {
					"type": "Record",
					"attributes": {
						"tags": {"type": "EntityOrCommon", "name": "Tags", "required": true}
					}
				}
			}
		},
	...
	// Verify the common type was resolved to SetType on the attribute
	userInfo := v.entityTypes["User"]
	if tagsAttr, ok := userInfo.Attributes["tags"]; !ok {
		t.Error("Attribute 'tags' not found on User")
	} else if st, ok := tagsAttr.Type.(schema.SetType); !ok {
		t.Errorf("Expected SetType for 'tags' (resolved from Tags common type), got %T", tagsAttr.Type)
...
func TestParseJSONTypeCommonTypeExtension(t *testing.T) {
		...
		"entityTypes": {
			"Server": {
				"shape": {
					"type": "Record",
					"attributes": {
						"ip": {"type": "EntityOrCommon", "name": "IPAddress", "required": true}
					}
				}
			}
		},
	...
	// Verify the common type was resolved to ExtensionType on the attribute
	serverInfo := v.entityTypes["Server"]
	if ipAttr, ok := serverInfo.Attributes["ip"]; !ok {
		t.Error("Attribute 'ip' not found on Server")
	} else if et, ok := ipAttr.Type.(schema.ExtensionType); !ok {
		t.Errorf("Expected ExtensionType for 'ip' (resolved from IPAddress common type), got %T", ipAttr.Type)
...
func TestParseJSONTypeCommonTypeEntityRef(t *testing.T) {
		...
		"entityTypes": {
			...
			"Document": {
				"shape": {
					"type": "Record",
					"attributes": {
						"owner": {"type": "EntityOrCommon", "name": "UserRef", "required": true}
					}
				}
			}
		},
	...
	// Verify the common type was resolved to EntityType on the attribute
	docInfo := v.entityTypes["Document"]
	if ownerAttr, ok := docInfo.Attributes["owner"]; !ok {
		t.Error("Attribute 'owner' not found on Document")
	} else if et, ok := ownerAttr.Type.(schema.EntityCedarType); !ok {
		t.Errorf("Expected EntityCedarType for 'owner' (resolved from UserRef common type), got %T", ownerAttr.Type)
...
func TestCanBeDescendantOfNilInfo(t *testing.T) {
	// Schema with an action that references an entity type in principalTypes.
	schemaJSON := `{
		"entityTypes": {"Document": {}, "UnknownType": {}},
		"actions": {
			"view": {
				"appliesTo": {
